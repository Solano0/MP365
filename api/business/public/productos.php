...
if (isset($_GET['action'])) {
    ...
    // se cargaran los productos para mostrar en las categorias
    switch ($_GET['action']) {
        case 'readProductossubCategoria':
            if (!$productos->setId($_POST['id_sub_categoria'])) {
                $result['exception'] = 'Categoría incorrecta';
            } elseif ($result['dataset'] = $productos->readProductossubCategoria()) {
                $result['status'] = 1;
